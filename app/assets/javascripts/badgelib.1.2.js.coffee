...
class UserButtonView extends Backbone.View
  ...
  show_stats_number: (evt) ->
    # hide whatever is there before
    canvas = @$('#credport-button-stats-canvas')
    canvas.remove()
    ...
    @$('#credport-button-stats').append canvas
    setTimeout ( -> 
      canvas.addClass 'fade-in'), 1
  ...
  render: ->
    ...
    if @show_identities_on_badge
      ...
      for identity in @model.get('verifications').identities
        @$('#credport-button-user-header-identities').append "<li><a href='#{identity.properties.url}'><img src='#{identity.image}' alt=''></a><span class='credport-button-user-header-identity-tooltip'>#{identity.subtitle}</span></li>"
    count = 0
    for k,v of @model.get('stats')
      if v.number != 0 and count < 3
        ...
        count++
    @renderStatsOverlays()
    # if @$('#credport-button-stats').children().length > 0
      # jQuery(@$('#credport-button-stats').children()[0]).addClass 'selected'
      # @$('#credport-button-stats-canvas').html(jQuery(@$('#credport-button-stats').children()[0]).find('.credport-button-stats-popup').clone())
    this
  renderStatsOverlays: ->
    for k,v of @model.get('stats')
      switch k
        ...
        when 'accounts'
          if @$('#credport-button-stats-accounts').length > 0
            ...
            for account in @model.get('verifications').identities.slice(0,10)
              @$('#credport-button-stats-popup-accounts-images').append("<li><a href='#{account.properties.url}'><img src='#{account.image}' alt=''></a></li>")
        when 'verifications'
